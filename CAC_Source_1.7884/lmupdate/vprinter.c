...
/*-----------------------------------------------------------------------*/
static void Put2Dec(int value, vpr_flagBits flagSet, PutRec *p)
{
	if (value >= 10)
		PutToS('0' + value / 10, p);
	else
	{
		if (flagSet & fillZerosBit)
			PutToS('0', p);
		else if (flagSet & leftJustBit)
			PutToS(' ', p);
	}
...
static void PutCSep(char sep, vpr_flagBits flagSet, PutRec *p)
{
	if (!(flagSet & altFormatBit))
		PutToS(sep, p);
}

static void PutZStr(const char *s, PutRec *p)
{
	while (*s)
		PutToS(*s++, p);
}
...
/*        _   A   B   C   D   E   F   G   H   I   J   K   L   M   N   O  */
	  _dc,_dc,_dc,_dc,_DT,_fl,_fa,_fl,_sh,_dc,_dc,_dc,_ld,_dc,_ne,_dc,

/*        P   Q   R   S   T   U   V   W   X   Y   Z   [   \   ]   ^   _  */
	  _dc,_dc,_dc,_dc,_TM,_dc,_dc,_dc,_he,_dc,_dc,_dc,_dc,_dc,_dc,_dc,
...
CopyLen:                                /* comes from %c or %s section  */
		...
		case (_DT) :
		{
			const DateValue *const date = va_arg(argP, const DateValue *);

			if (date == NULL)
				PutZStr(NullString, &put);
			else
			{
				Put2Dec(date->day, flagSet, &put);
				PutCSep('.', flagSet, &put);
				Put2Dec(date->month, flagSet, &put);
				PutCSep('.', flagSet, &put);
				/* always 4-digit year */
				Put2Dec(date->year / 100, flagSet | fillZerosBit, &put);
				Put2Dec(date->year % 100, flagSet | fillZerosBit, &put);
			}
		...
		case (_TM) :
		{
			const TimeValue *const time = va_arg(argP, const TimeValue *);

			if (time == NULL)
				PutZStr(NullString, &put);
			else
			{
				Put2Dec(time->hour, flagSet, &put);
				PutCSep(':', flagSet, &put);
				Put2Dec(time->minute, fillZerosBit, &put);

				if (flagSet & isLongBit)
				{
					PutCSep(':', flagSet, &put);
					Put2Dec(time->second, fillZerosBit, &put);
				}
			}
