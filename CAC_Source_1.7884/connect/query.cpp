...
const pFLDDesc TCACQuery::Find(const char *name) const
{
	int l = 0, h = count - 1;
...
bool TODBCQuery::GetStamp(const pFLDDesc desc, CDateTime *stamp) const
{
	if (states[desc->iFldNum] != SQL_NULL_DATA)
	{
		...
		stamp->day = times->day;
		stamp->month = times->month;
		stamp->year = times->year;
		stamp->hour = times->hour;
		stamp->minute = times->minute;
		stamp->second = times->second;
		return true;
	}
...
// ----- TBDEQuery -----------------------------------------------------------
TBDEQuery::TBDEQuery(const char *expr, TBDEDataBase *tDataBase, bool tSimple)
	: TCACQuery(expr), dataBase(tDataBase), simple(tSimple), statement(0), cursor(0)
{
	Open();
}
...
bool TBDEQuery::GetStamp(const pFLDDesc desc, CDateTime *stamp) const
{
	BOOL bIsNULL;
	TIMESTAMP times;
...
void TBDEQuery::Open()
{
	dataBase->Prepare(Name, &statement, simple);
	...
	if (cursor)
	{
		...
		descs = new FLDDesc[count];
	#if RANGECHECK
		if (simple && props.bTempTable)
			message("%s: bTempTable = %d", Name, (int) props.bTempTable);
	#endif  // RANGECHECK
...
// ----- TQuery --------------------------------------------------------------
TQuery::TQuery(const char *expr, TCACDataBase *dataBase, bool simple)
	: TObject(expr)
{
	query = (dataBase ? dataBase : TCACDataBase::DefaultDB)->NewQuery(expr, simple);
}
...
bool TQuery::GetStamp(const pFLDDesc desc, CDateTime *stamp) const
{
