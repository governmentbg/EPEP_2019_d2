...
bool TPostGresConnect::Load(char *value)
{
	...
	if (strlen(base) != 15 || strnicmp(base, "psqlodbc3", 9) || !isdigit(base[9]) || strnicmp(base + 10, "a.dll", 4))
		return false;
...
bool TPostGresConnect::Save(char *value)
{
	...
	*colon = '\0';
	...
	static constant params[] = { "CommLog=0", "Debug=0", "Fetch=100", "UniqueIndex=1", "UseDeclareFetch=1",
		"UnknownSizes=0", "TextAsLongVarchar=1", "UnknownsAsLongVarchar=0", "BoolsAsChar=1", "Parse=0",
		"MaxVarcharSize=255", "MaxLongVarcharSize=8190", "ExtraSysTablePrefixes=dd_;", "Description=", "Password=",
		"ReadOnly=0", "ShowOidColumn=0", "FakeOidIndex=0", "RowVersioning=0", "ShowSystemTables=0",
		"Protocol=7.4", "ConnSettings=", "UpdatableCursors=0", "LFConversion=0", "TrueIsMinus1=0",
		"BI=0", "AB=0", "ByteaAsLongVarBinary=0", "UseServerSidePrepare=1", "LowerCaseIdentifier=0",
		"GssAuthUseGSS=0", "SSLmode=allow", "KeepaliveTime=-1", "KeepaliveInterval=-1", "XaOpt=1",
		"Optimizer=0", "Ksqo=0", "CancelAsFreeStmt=0", "DisallowPremature=0", "PreferLibpq=-1" };
	...
	*colon = '\0';
	...
	static constant params_2[] = { "ReadOnly=0", "ShowOidColumn=0", "FakeOidIndex=0", "RowVersioning=0",
		"ShowSystemTables=0", "Protocol=7.4", "ConnSettings=", "DisallowPremature=0", "UpdatableCursors=0",
		"LFConversion=0", "TrueIsMinus1=0", "BI=0", "AB=0", "ByteaAsLongVarBinary=0", "UseServerSidePrepare=1",
		"LowerCaseIdentifier=0", "GssAuthUseGSS=0", "SSLmode=allow", "KeepaliveTime=-1", "KeepaliveInterval=-1",
...
{
	return TPostGresDataBase::DriverName();
}
...
static void ibase_fbird_syntax_warn(char *value, mstr &m)
{
	...
	if (!TFireBirdDataBase::BCCDriver())
	{
		m.sep(".\n\n");
		m.cat("Препоръчва се използването на ODBC BCC драйвера");
	}
}
...
bool TFireBirdConnect::Load(char *value)
{
	...
	if (strcmpi(base, "OdbcFBcc.dll") && strcmpi(base, "OdbcFB.dll"))
		return false;
...
{
	return TFireBirdDataBase::DriverName();
}
