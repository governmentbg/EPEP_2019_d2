...
// ----- mstr ----------------------------------------------------------------
class mstr : public memblock_t
{
public:
	mstr();
	explicit mstr(const char *s);
	explicit mstr(const mstr &m);
	~mstr() { mbk_clear(this); }

	char operator[](size_t n) const { return data[n]; }
	void cach(char c) { catn(&c, c != '\0'); }
	void cach_nx(char c);
	...
	int cmpi(const char *s) const { return strcmpi(str(), s); }
	int cmpi(const mstr &m) const { return cmp(m.str()); }
	...
	int index(const char *s) const { return strindex(str(), s); }
	...
	void sep_cat(const char *s, const char *t);
	void sep_cat(const char *s, const mstr &m) { sep_cat(s, m.str()); }
